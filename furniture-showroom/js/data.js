...
const CATEGORIES_SEED = [
  ...
  { id: 'corner-sofas', name: 'Corner Sofas', icon: 'fa-couch', desc: 'L-shaped and modular corner sofas for comfortable living spaces.', image: 'cat-corner' },
  ...
  { id: 'Libraries', name: 'Libraries', icon: 'fa-table-cells', desc: 'Vitrines, niches, and shelving to showcase your finest pieces.', image: 'cat-display' }
];
...
const PRODUCTS_SEED = [
  ...
  {
    id: 'p7', name: 'Milano Corner Sofa', category: 'corner-sofas', price: 38500, status: 'active', onOffer: true, discount: 15,
  ...
  {
    id: 'p15', name: 'Urban L-Sectional', category: 'corner-sofas', price: 31200, status: 'active',
  ...
  {
    id: 'p9', name: 'Verona Display Cabinet', category: 'Libraries', price: 24000, status: 'active',
  ...
  {
    id: 'p17', name: 'Gallery Wall Niche', category: 'Libraries', price: 15600, status: 'active', onOffer: true, discount: 12,
...
/* =============================================================== */
/* HERO SALE CAROUSEL — JS logic                                    */
/* Append to the BOTTOM of js/main.js                               */
/* =============================================================== */
/* Pulls all sale products (p.onOffer === true) from state.products */
/* and rotates them every 3 seconds with a fade transition.         */
/* Each slide is clickable → product page.                          */
/* If no sale products exist, the fallback image stays.             */
/* ================================================================= */

function initHeroSaleCarousel() {
  const carousel = document.getElementById('hero-sale-carousel');
  const dotsContainer = document.getElementById('hero-carousel-dots');
  if (!carousel) return;

  // Wait until state is loaded
  if (!window.state || !Array.isArray(window.state.products)) {
    setTimeout(initHeroSaleCarousel, 200);
    return;
  }

  // Get sale products
  const saleProducts = window.state.products.filter(p =>
    p.onOffer && p.discount && p.status === 'active' && p.images && p.images.length > 0
  );

  // No sale products? Keep the fallback image, hide dots.
  ...
  // Build slides
  const slidesHTML = saleProducts.map((p, i) => {
    const slug = p.slug || p.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    const imgUrl = (typeof productImageUrl === 'function')
      ? productImageUrl(p.images[0], 800, 1000)
      : p.images[0];
    return `
      <div class="hero-carousel-slide${i === 0 ? ' is-active' : ''}" data-index="${i}">
        <a href="products/${slug}.html" aria-label="${p.name} — on sale">
          <img src="${imgUrl}" alt="${p.name}" class="w-full h-full object-cover">
          <span class="hero-carousel-sale-badge">-${p.discount}%</span>
        </a>
      </div>
    `;
  }).join('');
  ...
  // Build dots
  if (dotsContainer) {
    dotsContainer.innerHTML = saleProducts.map((p, i) => `
      <button type="button" class="hero-carousel-dot${i === 0 ? ' is-active' : ''}"
              data-index="${i}" aria-label="Go to ${p.name}"></button>
    `).join('');
...
// Initialize after DOM is ready and other init has run
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => setTimeout(initHeroSaleCarousel, 300));
} else {
  setTimeout(initHeroSaleCarousel, 300);
}
